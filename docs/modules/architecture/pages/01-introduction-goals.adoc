...
|5
|*Extensibility*
|Pluggable LLM architecture via `LLMService` interface. Production backends: SKaiNET kllama (in-process JVM, embedded GGUF) and REST API (OpenAI-compatible — Ollama, LM Studio, OpenAI). Benchmark-only alternative engines (Deliverance, qxotic) plug into the same interface for engine-independence comparisons.
|===
