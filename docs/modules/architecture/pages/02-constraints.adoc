...
|*TC-01: JDK 25 toolchain*
|Required for the Java Vector API (incubator module, `--enable-preview`) and FFM native access (`--enable-native-access=ALL-UNNAMED`) used by the SKaiNET kernels. Gradle provisions the toolchain via the foojay resolver; JDK 21+ is enough to run Gradle itself.
...
|SKaiNET-transformers loads models in GGUF format with quantization (Q4, Q5, Q8, K-quants).
