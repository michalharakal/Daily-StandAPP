...
|Declarative UI, shares code with Android, native desktop rendering
...
*Solution*: Define `LLMService` interface for pluggable backends. Currently implemented with SKaiNET kllama - a pure Kotlin LLM implementation with SIMD acceleration via Java Vector API.
...
classDiagram
    ...
    LLMService <|.. SkainetKLlamaService
    LLMService <|.. FutureBackend
----
